...
Thredded.user_name_column = :ig_username
...
Thredded.user_path = ->(user) {
  ...
  main_app.respond_to?(user_path) ? main_app.send(user_path, user.ig_username) : "/users/#{user.ig_username}"
}
...
Thredded.avatar_url = ->(user) { user.photo.thumb.url(156, 'mm') }
...
Thredded.email_from = 'hello@inbedwithbetty.com'
